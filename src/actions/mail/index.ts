...
export const onCreateMarketingCampaign = async (name: string) => {
  try {
    ...
    if (campaign) {
      return { status: 200, message: 'Ваша кампания успешно создана' }
    }
  } catch (error) {
    console.log(error)
  }
...
export const onSaveEmailTemplate = async (
  ...
  try {
    ...
    return { status: 200, message: 'Создан шаблон электронной почты' }
  } catch (error) {
    console.log(error)
  }
}
...
export const onAddCustomersToEmail = async (
  ...
  try {
    ...
    if (customerAdd) {
      return { status: 200, message: 'Клиент, добавлен в кампанию' }
    }
  } catch (error) {}
}
    ...
    if (template && template.template) {
      ...
      transporter.sendMail(mailOptions, function (error, info) {
        if (error) {
          ...
          console.log('Отправлено электронное письмо: ' + info.response)
        }
      })
      ...
      if (creditsUsed) {
        return { status: 200, message: 'Сообщения были разосланы' }
      }
    }
  } catch (error) {
    console.log(error)
