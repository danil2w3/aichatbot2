...
export const useFilterQuestions = (id: string) => {
  ...
  const { toast } = useToast()
  const [loading, setLoading] = useState<boolean>(false)
  const [isQuestions, setIsQuestions] = useState<{ id: string; question: string; answered: string | null }[]>([])
  const onAddFilterQuestions = handleSubmit(async (values) => {
    setLoading(true)
    const questions = await onCreateFilterQuestions(id, values.question, values.answered)
    if (questions) {
      setIsQuestions(questions.questions!)
      toast({
